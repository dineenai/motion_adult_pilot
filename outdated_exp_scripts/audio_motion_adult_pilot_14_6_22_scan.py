...
def run_trial(win, audio, aud_file, save_aud_status_loc, MR_settings, save_loc, Session_Info, subNum):
    ...
    if sync_now:    # waits for sync 
        ...
        # audio.play(when=ptb.GetSecs()+nextFlip+audio_delay) 
        # audio.play(when=audio_delay) 
        audio.play(when=nextFlip+audio_delay) 
        # audio.play(when=5) 

        # # Turn circle white to start moving/button pressing
        ...
        # preaudio_met_dur = 18.000
        # This should be the same as audio delay!!!!
        preaudio_met_dur = 10.000
