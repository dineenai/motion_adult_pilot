    ...
    if sync_now:    # waits for sync 
        ...
        for thisComponent in trialComponents:
            ...
            if hasattr(thisComponent, 'status'):
                ...
                print(f'THIS PRINTS IF THE CIRCLE HAS ATTR STATUS!!! {thisComponent}, {thisComponent.status}')
        # reset timers
        t = 0
        _timeToFirstFrame = win.getFutureFlipTime(clock="now")
        print(f'_timeToFirstFrame is: {_timeToFirstFrame} ')
        ...
        print(f"START OF EXPERIMENT: {ptb.GetSecs()}")
        ...
        print(f"REAL AUDIO START????? {nextFlip}")
        ...
        while continueRoutine > 0:
            ...
            t_start = ptb.GetSecs()
            # print(f't at Start of loop : {t_start}')
            ...
            if continueRoutine:  # don't flip if this routine is over or we'll get a blank screen
                ...
                end_pre_flip_time = ptb.GetSecs()
                
                # remove sleep for now
                # time.sleep(0.005)
        ...
        df.to_csv(f'win_flip_{id}.csv') 
        df2.to_csv(f'win_flip_{id}_loop.csv') 
